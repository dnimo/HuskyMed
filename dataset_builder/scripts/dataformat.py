    ...
    for _, items in df.iterrows():
        ...
        for item in items["smrs"]:
            if item["smr"] is not None:
                smrs_list.append(
                    {
                        "INPUT_DATE": datetime.strptime(item["INPUT_DATE"], "%Y-%m-%d"),
                        "smr_type": item["EMR_TYPE"],
                        "section": item["SECTION"],
                        "Unit": [
                            {"data_type": value["EMR_DATA_TYPE"], "text": value["EMR_TEXT"]}
                            for value in item["smr"]
                        ],
                    }
                )
        for record in items["records"]:
            ts = datetime.strptime(record["ts"][:8], "%Y%m%d")
            refs_list.append({"INPUT_DATE": ts, "section": record["section"], "Unit": record["emr"]})
        ...
        for smr in smrs_list:
            ...
            reform.append({"ref": new_refs, "section": smr["section"], "smr_type": smr["smr_type"], "target": smr["Unit"]})
